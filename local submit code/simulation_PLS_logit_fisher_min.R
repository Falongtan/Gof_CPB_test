# PLS_logistic
###test whether a logistic model is properly formulated
###use min and fisher combination method to combine the p-values on half of the data, respectively
###use the Cauchy combination method to combine the p-values from two data (crossfitting)
###compare with grp test
PLS_logit_parallel <- function(p,n,a,c_h,pho,example,rpgrp_switch){
  library(MASS)
  library(glmnet)
  library(LassoSIR) 
  ...
  PLS_power <- foreach(k = 1:s, .combine='rbind', 
                       .packages = c('MASS','glmnet','LassoSIR','VariableScreening','psych',
                                     'foreach','parallel','iterators','doParallel','GRPtests')) %dopar%  
    {
      ...
      result <- c(pval_fisher1,pval_fisher2,pval_fisher,
                  pval_min1,pval_min2,pval_min,pval_grp)
    }
  #end parallel
  stopImplicitCluster()
  stopCluster(cl)
  ...
  PLS_grp_power <- mean(PLS_pval_grp<=0.05,na.rm=TRUE)
  ...
  return(list(PLS_fisher1_power=PLS_fisher1_power,
              ...
              PLS_grp_power=PLS_grp_power))
}



#####analysis of Empirical sizes and powers of our tests with different bandwidths in logistic models
example <- c(1,2) 
    ...
    for (ii in 1:length(a)){
      for (jj in 1:length(p)){
        for (cc in 1:length(c_h)){
          ...
          t3 <- Sys.time()
          result_pls <- PLS_logit_parallel(p[jj], n, a[ii],c_h[cc],pho[kk],example[ll],rpgrp_switch=rpgrp_run) 
          #index
          ...
          cat("example = ",example[ll],"p = ", p[jj],", n = ", n, ", a = ", a[ii] ,", c_h = ", c_h[cc], ", pho = ", pho[kk],
              ...
              ", grp_power = ",  result_pls$PLS_grp_power, 
              ", time:",t4-t3, "\n")
